...
-- ===== 获取可用道具列表 =====
function ulx.getAvailableItems()
...
if SERVER then
	...
	net.Receive( "ulx_items_spawn", function( len, ply )
		...
		for _, target in ipairs( targets ) do
			...
			local pos = trace.HitPos + Vector( 0, 0, 20 )
			...
			for i = 1, quantity do
				...
				if not IsValid( ent ) then
					...
					if vdata and vdata.KeyValues then
						for k, v in pairs( vdata.KeyValues ) do
							ent:SetKeyValue( k, v )
						end
					end
					...
					spawned = spawned + 1
					pos = pos + Vector( math.random( -50, 50 ), math.random( -50, 50 ), 10 )
				end
			end
		end
