...
------------------------------ MapVote 兼容指令 ------------------------------
-- 原 MapVote addon 遗留的注册访问权限，整合为正式命令
function ulx.mapvote( calling_ply )
...
mapvoteCmd:help( "发起地图投票（兼容 MapVote 指令）。" )
mapvoteCmd:setOpposite( "ulx unmapvote", {}, "!unmapvote" )

function ulx.gmvote( calling_ply )
	if not calling_ply:IsValid() then
		Msg( "无法从控制台使用 gmvote。\n" )
		return
	end
...
gmvoteCmd:help( "发起游戏模式投票（兼容 MapVote 指令）。" )
gmvoteCmd:setOpposite( "ulx ungmvote", {}, "!ungmvote" )
