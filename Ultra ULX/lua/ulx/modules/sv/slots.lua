...
function calcSlots( disconnect )
	...
	if mode == 1 then
		...
		if disconnect then admins = admins - 1 end
		if admins < 0 then admins = 0 end
		local rslots = slots - admins
		if rslots < 0 then rslots = 0 end
		return rslots
	end
	return 0
end
...
local function playerAccess( ply )
	...
	local max = game.MaxPlayers()
	if ply:query( access ) then
		if not visible then
			updateSlots()
		end
		if mode == 3 and cur + slots > max then
			local shortestply
	...
	if cur + slots > max then
		ULib.queueFunctionCall( ULib.kick, ply, "[ULX] Reserved slot, sorry!" )
	end
end
hook.Add( ULib.HOOK_UCLAUTH, "ULXReservedSlots", playerAccess, HOOK_MONITOR_LOW )