...
for _, file in ipairs( xgui_settings ) do
	AddCSLuaFile( "ulx/xgui/settings/" .. file )
	Msg( "//  " .. file .. string.rep( " ", 25 - file:len() ) .. "//\n" )
end
Msg( "// Adding Gamemode Modules.. //\n" )
local xgui_gamemodes = { "sandbox.lua" }
for _, file in ipairs( xgui_gamemodes ) do
...
function xgui.init()
	...
	function xgui.sendDataTable( plys, datatypes, forceSend )
		...
		for k, ply in pairs( plys ) do
			if not xgui.readyPlayers[ply:UniqueID()] then return end
			if xgui.activeUsers[ply:UniqueID()] and not forceSend then
			...
			for _, dtype in ipairs( datatypes ) do
				if xgui.dataTypes[dtype] then
					...
					if ULib.ucl.query( ply, data.access ) then
						...
						local size = data.maxchunk or 0
						if t and table.Count( t ) > size and size ~= 0 then
							table.insert( chunks, { 5, dtype } )
							local c = 1
							local part = {}
