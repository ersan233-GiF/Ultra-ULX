...
local function cc_ulx( ply, command, argv )
	...
	if argn == 0 then
		...
		ULib.console( ply, "无效命令。如需帮助，请在控制台输入 \"ulx help\"。" )
	end
end
ULib.cmds.addCommand( "ulx", cc_ulx )
function ulx.help( ply )
	local isPlayer = ( ply and ply:IsValid() and ply:IsPlayer() )
	for category, cmds in pairs( ulx.cmdsByCategory ) do
		...
		if #lines > 0 then
			...
			for _, line in ipairs( lines ) do
				if #msg + #line > 240 then
					if isPlayer then ULib.tsay( ply, msg ) else ULib.console( ply, msg ) end
					msg = "  "
				end
				msg = msg .. line .. "  |  "
			end
	...
	local footer = "ULX v" .. ULib.pluginVersionStr( "ULX" ) .. "  - 共 " .. totalCmds .. " 条命令"
	if isPlayer then ULib.tsay( ply, footer ) else ULib.console( ply, footer ) end
end
local help = ulx.command( "工具", "ulx help", ulx.help )
help:help( "显示此帮助信息。" )
